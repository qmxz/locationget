...
output_sheet.write(0, 1, 'current address')
output_sheet.write(0, 2, 'longitude')
output_sheet.write(0, 3, 'latitude')
...
def process_data(row_index):
    ...
    # 构建API请求的参数字符串
    params_str = f'key={api_key}&address={cell_value_1}&output=XML'
 ...
 # 解析XML数据获取所需的值
    ...
    output_sheet.write(row_index, 1, target_value1)
    output_sheet.write(row_index, 2, longitude)
    output_sheet.write(row_index, 3, latitude)
